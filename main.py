...
PdfDirOption = Annotated[
    Path,
    typer.Option(
        "--pdf-dir", help="Directory for downloading and accessing PDF reports."
    ),
]
...
def run_all_pipeline(
    ...
    async def _run_all():
        ...
        try:
            ...
            typer.echo(f"Error: Failed to connect to or configure the database. Details: {db_err}", err=True)
            raise typer.Exit(code=1)
            ...
            typer.echo(f"Error: Configuration issue (e.g., missing API token, invalid value). Details: {config_err}", err=True)
            raise typer.Exit(code=1)
