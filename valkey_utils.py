    ...
    def __init__(
        ...
        try:
            ...
            raise ConnectionError(
                f'Failed to connect to or ping Valkey instance at {host}:{port}'
            ) from e
        except Exception as e:
            raise ConnectionError(
                f'An unexpected error occurred during Valkey client initialization for {host}:{port}'
            ) from e
    ...
    def new() -> 'ValkeyClient':
        ...
        try:
            ...
            raise ConfigurationError(
                f'Failed to connect to Valkey with derived settings: {ce}'
            ) from ce
        except ConfigurationError as conf_err:
            raise conf_err
        except ValueError as ve:
            raise ConfigurationError(
                f'Invalid numeric value provided in environment variable: {ve}'
            ) from ve
